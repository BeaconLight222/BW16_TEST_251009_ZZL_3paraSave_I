...
void AwsMqtt::begin() {
  ...
  wifiReset    = false;
  factoryReset = false;
  diagnosticsLevel    = 9;      //deqing created data
...
void AwsMqtt::loop() {
  ...
  case WAIT_FOR_JOB_QUERY_RESPONSE: {
    ...
    for (int i = (int)(jobsMessages.size()) - 1; i >= 0; i--) {
      if (strstr(jobsMessages[i].topic.c_str(), "/jobs/get/accepted") != NULL) {
        ...
        if (!error) {
          ...
          if (doc["inProgressJobs"].is<JsonArray>()) {
            ...
            for (JsonVariant job : inProgressJobs) {
              ...
              }//end  if (job.is<JsonObject>())
            }//end for
          }//end if (doc["inProgressJobs"]
        ...
        jobsMessages.erase(jobsMessages.begin() + i);   // +i NOT  +1 !
      }//end    if (strstr(jobsMessages[i].topic
    }//end    for (int i = (int)(jobsMessages.size())
  ...
  case DO_THE_JOB: {
    ...
    //bool firmwareAutoUpdate =  payload_manager.payload_deviceConfig_sub.firmwareAutoUpdate;

    if (  firmwareAutoUpdate==true &&      processingOTAFileSize > 0 && processingOTAStreamName.length() > 0 &&
...
void AwsMqtt::serverCallback(char *topic, char *payload, unsigned int length) {
  ...
  if ((strstr(topic, "/command/sub") != NULL)) {
    ...
    if (doc["detectionMode"].is<int>()) {
      ...
      payload_manager.payload_command_sub.detectionMode = (uv_lamp_mode_enum)mode;
      
      if(fprint){
      Serial.print("detectionMode:");
      Serial.println( payload_manager.payload_command_sub.detectionMode);
      }

      if (mode == 0) {
        // this is smart mode
        payload_manager.list_newModeCommand.push_back("UV_MODE_SMART");
        ...
        if (doc["lampEnable"].is<bool>()) {
          ...
          payload_manager.payload_command_sub.lampEnable = lampEnable;
          //payload_manager.payload_command_sub.lampEnable = lampEnable? true:false;
    
          if(fprint){
            Serial.print("lampEnable:");
            Serial.println(payload_manager.payload_command_sub.lampEnable);
          }
        ...
        }//end if (doc["lampEnable"]
      }else if (mode ==2){
                payload_manager.list_newModeCommand.push_back("UV_MODE_UNLIMITED");
      }else if (mode ==3){
                payload_manager.list_newModeCommand.push_back("UV_MODE_OCCUPIED_ROOM");
      }else if (mode ==4){
                payload_manager.list_newModeCommand.push_back("UV_MODE_EMPTY_ROOM");
      }       
         
    }//end if (doc["detectionMode"]


    if (doc["factoryReset"].is<bool>()) {
      factoryReset   = doc["factoryReset"];
      payload_manager.payload_command_sub.factoryReset  = doc["factoryReset"];

      if(fprint){
      Serial.print("AwsMqtt::factoryReset:"); Serial.println(factoryReset );
      Serial.print("payload_command_sub.factoryReset");Serial.println(payload_manager.payload_command_sub.factoryReset);
      }

      if (factoryReset) {
          if(fprint){
              Serial.println("Server Command factoryReset");
          }
        payload_manager.list_newModeCommand.push_back("RESET_WIFI");
      }else{
          if(fprint){
             Serial.println("Server Command factoryNormal");
          }
      }//end    else

    }//end  if (doc["factoryReset"]


    if (doc["wifiReset"].is<bool>()) {
       wifiReset = doc["wifiReset"];
       payload_manager.payload_command_sub.wifiReset = doc["wifiReset"];  

        if(fprint){
          Serial.print("AwsMqtt::wifiReset");Serial.println(wifiReset);
          Serial.print("payload_command_sub.wifiReset:");
      ...
      if (wifiReset) {
        ...
        if(fprint){
          Serial.println("Server Command wifiNormal");
        }  
      }//end else
    }//end      if (doc["wifiReset"].is<bool>())
    ...
    if (doc["scheduleEnable"].is<bool>()) {
          ...
          payload_manager.payload_command_sub.scheduleEnable = scheduleEnabled;
        //payload_manager.payload_command_sub.scheduleEnable = scheduleEnabled? true:false;

        if(fprint){
            Serial.print("AwsMqtt::scheduleEnable:");Serial.println(scheduleEnabled);
            Serial.print("payload_manager.payload_command_sub.scheduleEnable:");Serial.println(payload_manager.payload_command_sub.scheduleEnable);
        }//end    if(fprint)

        payload_manager.list_newModeCommand.push_back("JOB_SCHEDULE_ENABLE");
    
    }//end    if (doc["scheduleEnable"].is<bool>())



    if (doc["resetAccumulatedExposureLimit"].is<bool>()) {
        resetAccumulatedExposureLimit = doc["resetAccumulatedExposureLimit"];
        payload_manager.payload_command_sub.resetAccumulatedExposureLimit = resetAccumulatedExposureLimit;
        
        if(fprint){
          Serial.print("AwsMqtt::resetAccumulatedExposureLimited:"); Serial.println(resetAccumulatedExposureLimit);
          Serial.print("payload_command_sub.resetAccumulatedExposureLimit");Serial.println(payload_manager.payload_command_sub.resetAccumulatedExposureLimit);
        }

      payload_manager.list_newModeCommand.push_back("JOB_RESET_ACCUMULATED_EXPOSURE_LIMIT");

    }//end    if (doc["resetAccumulatedExposureLimit"]
    
    if (doc["setPersonCountThreshold"].is<int>()) {
      setPersonCountThreshold  = doc["setPersonCountThreshold"];
      payload_manager.payload_command_sub.setPersonCountThreshold = setPersonCountThreshold;

      if(fprint){
        Serial.print("AwsMqtt::setPersonCountThreshold:"); Serial.println(setPersonCountThreshold);
        Serial.print("payload_command_sub.setPersonCountThreshold:"); Serial.println(payload_manager.payload_command_sub.setPersonCountThreshold);

      }
    }//end if

    if (doc["enableDiagnosticsMode"].is<bool>()) {
        enableDiagnosticsMode = doc["enableDiagnosticsMode"];
        payload_manager.payload_command_sub.enableDiagnosticsMode = enableDiagnosticsMode;

        if(fprint){
          Serial.print("enableDiagnosticsMode:"); Serial.println(enableDiagnosticsMode);
          Serial.print("payload_command_sub.enableDiagnosticsMode"); Serial.println( payload_manager.payload_command_sub.enableDiagnosticsMode);
        }
    }

    if (doc["muteAlarms"].is<bool>()) {
        muteAlarms =    doc["muteAlarms"];
        payload_manager.payload_command_sub.muteAlarms = muteAlarms;
        payload_manager.list_newModeCommand.push_back("JOB_MUTE_ALARMS");

        if(fprint){
           Serial.print("AswMqtt::muteAlarms:");Serial.println(muteAlarms);
           Serial.print("payload_command_sub.muteAlarms:"); Serial.println( payload_manager.payload_command_sub.muteAlarms );
        }

    }//end    if (doc["muteAlarms"].is<bool>())



    if (doc["rebootDevice"].is<bool>()) {

      rebootDevice = doc["rebootDevice"];
      payload_manager.payload_command_sub.rebootDevice = rebootDevice; 

      if(fprint){
        Serial.print("AwsMqttrebootDevice:"); Serial.println(rebootDevice);
        Serial.print("payload_command_sub.rebootDevice:"); Serial.println(payload_manager.payload_command_sub.rebootDevice);
      } 

      if(rebootDevice){
        if(fprint){
            Serial.println("Server Command rebootDevice");
        }
        payload_manager.list_newModeCommand.push_back("RESET_WIFI");
      }else{
        if(fprint){
            Serial.println("Server Command NO rebootDevice");
        }
      }//end else

      
    }//end if (doc["rebootDevice"]

    if (doc["telemetryIntervalSeconds"].is<int>()) {
      telemetryIntervalSeconds = doc["telemetryIntervalSeconds"];
      payload_manager.payload_command_sub.telemetryIntervalSeconds = doc["telemetryIntervalSeconds"];

      if(fprint){
        Serial.print("AwsMqtt::telemetryIntervalSeconds:"); Serial.println(telemetryIntervalSeconds);
        Serial.print("payload_command_sub.telemetryIntervalSeconds:"); Serial.println(payload_manager.payload_command_sub.telemetryIntervalSeconds);
      }  
    
    }//end    if (doc["telemetryIntervalSeconds"]

     if (doc["setProxyIp"].is<String>()) {
 //           setProxyIp = doc["setProxyIp"];
          String setProxyIp = doc["setProxyIp"];
          this->  setProxyIp = setProxyIp;
          payload_manager.payload_command_sub.setProxyIp = setProxyIp;

//            if(true){
          if(fprint){
            Serial.print("AwsMqtt:setProxyIp:");Serial.println(this->setProxyIp);
            Serial.print("payload_command_sub.setProxyIp:"); Serial.println(payload_manager.payload_command_sub.setProxyIp); 
          }

    }//end    if (doc["setProxyIp"]    

        if (  doc["proxyEnabled"].is<bool>()  ) {
          proxyEnabled = doc["proxyEnabled"];
          payload_manager.payload_command_sub.proxyEnable = proxyEnabled;

          //bool proxyEnabled = doc["proxyEnabled"];
          //payload_manager.payload_command_sub.proxyEnable = proxyEnabled? true:false;

          //if(true){
          if(fprint){
              Serial.print("AwsMqtt::proxyEnabled:"); Serial.println(proxyEnabled);
              Serial.print("payload_command_sub.proxyEnable"); Serial.println(payload_manager.payload_command_sub.proxyEnable);
            }
        }//end    if (doc["proxyEnabled"]

        //thresAccumulatedExposure":161  
        if(   doc["thresAccumulatedExposure"].is<int>() ){
          thresAccumulatedExposure = doc["thresAccumulatedExposure"];
          //int thresAccumulatedExposure = doc["thresAccumulatedExposure"];
          payload_manager.payload_command_sub.thresAccumulatedExposure = thresAccumulatedExposure;
          payload_manager.list_newModeCommand.push_back("JOB_THRES_ACCUMULATED_EXPOSURE");

          //if(true){
          if(fprint){
            Serial.print("AwsMqtt::thresAccumulatedExposure:");Serial.println(thresAccumulatedExposure);
            Serial.print("payload_command_sub.thresAccumulatedExposure:");Serial.println(payload_manager.payload_command_sub.thresAccumulatedExposure);
          }

        }//end    if(   doc["thresAccumulatedExposure"].is<int>() )
  //  if(true){
  if(fprint){
    ...
    if(true){
      Serial.println("_______________________________ deviceConfig/sub is received____________________________________");
    }
    ...
    if (doc["rtcUnixTime"].is<int>()) {
      rtcUnixTime = doc["rtcUnixTime"];
      payload_manager.payload_deviceConfig_sub.rtcUnixTime = rtcUnixTime; 

      if(fprint){
         Serial.print("Server Config rtcUnixTime: ");Serial.print("AwsMqttrtcUnixTime:");Serial.println(rtcUnixTime);
        Serial.print("payload_deviceConfig_sub.rtcUnixTime:"); Serial.println(payload_manager.payload_deviceConfig_sub.rtcUnixTime);
      }

     }//end   if (doc["rtcUnixTime"].is<int>())


  if (doc["schedule"].is<JsonArray>()) {
    //scheduleArr = doc["schedule"];
      ...
      for (JsonVariant v : scheduleArr) {
        ...
        }//end    if (byteIndex < sizeof(scheduleData))
        idx++;
      }//end    for (JsonVariant v : scheduleArr)
...
//        if (byteIndex < sizeof(42)) {
          if (data) {
            payload_manager.payload_deviceConfig_sub.scheduleData[byteIndex] |= (1 << bitIndex);
          } else {
            payload_manager.payload_deviceConfig_sub.scheduleData[byteIndex] &= ~(1 << bitIndex);
          }
        }
      ...
      if(fprint){
        ...
        for (unsigned int i = 0; i < sizeof(  payload_manager.payload_deviceConfig_sub.scheduleData ); i++) {
          ...
          Serial.print(" ");
        }//end        for (int i = 0; i < sizeof(42); i++)
          Serial.println();
      }//end  if(fprint)


      payload_manager.list_newModeCommand.push_back("JOB_SCHEDULE");
    }//end          if (doc["schedule"]
    ...
    if (doc["timeZoneOffsetMinutes"].is<int>()) {
      ...
      payload_manager.payload_deviceConfig_sub.timeZoneOffsetMinutes = doc["timeZoneOffsetMinutes"];

      payload_manager.list_newModeCommand.push_back("JOB_TIME_ZONE_OFFSET_MINUTES");


      if(fprint){
      Serial.print("Server Config timeZoneOffsetMinutes: AwsMqtt::");Serial.println(scheduleTimezoneOffsetMinutes);
      Serial.print("payload_deviceConfig_sub.timeZoneOffsetMinutes:");Serial.println(payload_manager.payload_deviceConfig_sub.timeZoneOffsetMinutes);
      }      
    
     // payload_manager.list_newModeCommand.push_back("JOB_TIMEZONE");
     }//end    if (doc["timeZoneOffsetMinutes"].is<int>())

  if(doc["configVersion"].is<String>()) {
    String  configVersion  = doc["configVersion"];
    this->configVersion = configVersion;
    payload_manager.payload_deviceConfig_sub.configVersion = configVersion;

      if(fprint){
//    if(true){
    Serial.print("AwsMqtt::configVersion:");    Serial.println(this->configVersion);
    Serial.print("payload_deviceConfig_sub.configVersion:"); Serial.println(payload_manager.payload_deviceConfig_sub.configVersion);
    }

  }//end      if(doc["configVersion"]


  if(doc["scheduleVersion"].is<uint8_t>()) {
    scheduleVersion = doc["scheduleVersion"];
    payload_manager.payload_deviceConfig_sub.scheduleVersion = doc["scheduleVersion"];

//      if(true){
    if(fprint){
      Serial.print("AwsMqtt::scheduleVersion:"); Serial.println(scheduleVersion);
      Serial.print("payload_deviceConfig_sub.scheduleVersion:");Serial.println(payload_manager.payload_deviceConfig_sub.scheduleVersion);
    }

  } //end   if(doc["scheduleVersion"]
  
  if(doc["defaultDetectionMode"].is<uint8_t>()){
     defaultDetectionMode =  doc["defaultDetectionMode"];
     payload_manager.payload_deviceConfig_sub.defaultDetectionMode = doc["defaultDetectionMode"];
     payload_manager.list_newModeCommand.push_back("JOB_DEFAULT_DETECTION");

      if(fprint){
      //if(true){
          Serial.print("AwsMqtt::defaultDetectionMode:");Serial.println(defaultDetectionMode);
          Serial.print("payload_deviceConfig_sub.defaultDetectionMode:");Serial.println(payload_manager.payload_deviceConfig_sub.defaultDetectionMode);
      }

     
  }//end    if(doc["defaultDetectionMode"]

  if(doc["lampStartupState"].is<String>()){
    String  lampStartupState =doc["lampStartupState"];
    this->lampStartupState = lampStartupState;
    payload_manager.payload_deviceConfig_sub.lampStartupState     = lampStartupState;
    payload_manager.list_newModeCommand.push_back("JOB_LAMP_STARTUP_STATE");

    if(fprint){
    //if(true){
      Serial.print("AwsMqtt:lampStartupState:");Serial.println(lampStartupState);
      Serial.print("payload_deviceConfig_sub.lampStartupState:");Serial.println(payload_manager.payload_deviceConfig_sub.lampStartupState);
    }
    
  }//end      if(doc["lampStartupState"].is<String>())

  if (doc["telemetryIntervalSeconds"].is<int>() ){
      telemetryIntervalSeconds=doc["telemetryIntervalSeconds"];
      payload_manager.payload_deviceConfig_sub.telemetryIntervalSeconds = doc["telemetryIntervalSeconds"];

      if(fprint){
        Serial.print("AwsMqtt::telemetryIntervalSeconds:");Serial.println(telemetryIntervalSeconds);
        Serial.print("payload_deviceConfig_sub.telemetryIntervalSeconds:");Serial.println(payload_manager.payload_deviceConfig_sub.telemetryIntervalSeconds);
      }
  }//end    if ((doc["telemetryIntervalSeconds"]

  if( doc["firmwareAutoUpdate"].is<bool>() ){
    firmwareAutoUpdate  = doc["firmwareAutoUpdate"];
    payload_manager.payload_deviceConfig_sub.firmwareAutoUpdate = doc["firmwareAutoUpdate"];

    if(fprint){
      Serial.print("AwsMqtt::firmwareAutoUpdate:"); Serial.println(firmwareAutoUpdate);
      Serial.print("payload_deviceConfig_sub.firmwareAutoUpdate:");Serial.println(payload_manager.payload_deviceConfig_sub.firmwareAutoUpdate);
    }

  }//end

  if( doc["diagnosticsLevel"].is<uint8_t>()   ){
    diagnosticsLevel =  doc["diagnosticsLevel"];
    payload_manager.payload_deviceConfig_sub.diagnosticsLevel = doc["diagnosticsLevel"];

    if(fprint){ 
      Serial.print("AwsMqtt::diagnosticsLevel:");Serial.println(diagnosticsLevel);
      Serial.print("payload_deviceConfig_sub.diagnosticsLevel:");Serial.println(payload_manager.payload_deviceConfig_sub.diagnosticsLevel);
    }//end    if(fprint)
  
  }//end        if( doc["diagnosticsLevel"]

  }//end          else if ((strstr(topic, "/deviceConfig/sub")           

}//end        void AwsMqtt::serverCallback
...
//      payload_manager.payload_command_sub.factoryReset = factoryReset? true:false;
    ...
    if (doc["wifiReset"].is<bool>()) {
       wifiReset = doc["wifiReset"];
       Serial.print("wifiReset");Serial.println(wifiReset);
    ...
    if (doc["scheduleEnable"].is<bool>()) {
      ...
      //Serial.println(scheduleEnabled);
        ...
        Serial.println(payload_manager.payload_command_sub.scheduleEnable);

    }

    if (doc["resetAccumulatedExposureLimit"].is<bool>()) {
      resetAccumulatedExposureLimit = doc["resetAccumulatedExposureLimit"];
        payload_manager.payload_command_sub.resetAccumulatedExposureLimit = resetAccumulatedExposureLimit? true:false;
        //Serial.print("resAcc:");
        //Serial.println(resetAccumulatedExposureLimit);
        Serial.print("resetAccumulatedExposureLimited:");
        Serial.println(payload_manager.payload_command_sub.resetAccumulatedExposureLimit);
    }
    ...
    if (doc["setPersonCountThreshold"].is<int>()) {
      ...
      payload_manager.payload_command_sub.setPersonCountThreshold = setPersonCountThreshold;
      Serial.print("setPersonCountThreshold:");
      Serial.println(payload_manager.payload_command_sub.setPersonCountThreshold);
    }//end if

    if (doc["enableDiagnosticsMode"].is<bool>()) {
        enableDiagnosticsMode = doc["enableDiagnosticsMode"];
        payload_manager.payload_command_sub.enableDiagnosticsMode = enableDiagnosticsMode;
        Serial.print("enableDiagnosticsMode:");
        Serial.println( payload_manager.payload_command_sub.enableDiagnosticsMode);
    }

    if (doc["muteAlarms"].is<bool>()) {
        muteAlarms =    doc["muteAlarms"];
        payload_manager.payload_command_sub.muteAlarms = muteAlarms;
        Serial.print("muteAlarms:");
        Serial.println( payload_manager.payload_command_sub.muteAlarms);

         payload_manager.list_newModeCommand.push_back("JOB_MUTE_ALARMS");
       // payload_manager.list_newModeCommand.push_back("JOB_MUTE_ALARAMS");
    }
    ...
    if (doc["rebootDevice"].is<bool>()) {
      ...
      Serial.println(payload_manager.payload_command_sub.rebootDevice);
    ...
    if (doc["telemetryIntervalSeconds"].is<int>()) {
      telemetryIntervalSeconds = doc["telemetryIntervalSeconds"];
      Serial.print("telemetryIntervalSeconds:");
      Serial.println(telemetryIntervalSeconds);

      payload_manager.payload_command_sub.telemetryIntervalSeconds = doc["telemetryIntervalSeconds"];
//    payload_manager.payload_command_sub.telemetryIntervalSeconds = telemetryIntervalSeconds;
      Serial.print("payload_command_sub.telemetryIntervalSeconds:");
      Serial.println(payload_manager.payload_command_sub.telemetryIntervalSeconds);
        ...
        if (doc["setProxyIp"].is<String>()) {
          ...
          Serial.print("payload_command_sub.setProxyIp:");
          Serial.println(payload_manager.payload_command_sub.setProxyIp); 
    }//end    if (doc["setProxyIp"]    

        if (  doc["proxyEnabled"].is<bool>()  ) {
          bool proxyEnabled = doc["proxyEnabled"];
          payload_manager.payload_command_sub.proxyEnable = proxyEnabled? true:false;
          Serial.print("proxyEnabled:");
          Serial.println(payload_manager.payload_command_sub.proxyEnable);
        }//end    if (doc["proxyEnabled"]

        //thresAccumulatedExposure":161  
        if(   doc["thresAccumulatedExposure"].is<int>() ){
          int thresAccumulatedExposure = doc["thresAccumulatedExposure"];
          Serial.print("thresAccumulatedExposure:");Serial.println(thresAccumulatedExposure);
  ...
  for (int n=0; n< payload_manager.list_newModeCommand.size() ; n++){
   ...
   Serial.print("\n");
    ...
    if (doc["rtcUnixTime"].is<int>()) {
      ...
      payload_manager.payload_deviceConfig_sub.rtcUnixTime = rtcUnixTime; 
      Serial.print("payload_deviceConfig_sub.rtcUnixTime:");
      Serial.println(payload_manager.payload_deviceConfig_sub.rtcUnixTime);
    }
...
//        if (byteIndex < sizeof(42)) {
  ...
  if(doc["configVersion"].is<String>()) {
    //this->configVersion      = doc["configVersion"];

    String  configVersion  = doc["configVersion"];
    this->configVersion = configVersion;

    //configVersion  =(char*) doc["configVersion"];
    //configVersion  = (String)doc["configVersion"];
    //configVersion  = doc["configVersion"];
    Serial.print("configVersion:");
    Serial.println(configVersion);

    payload_manager.payload_deviceConfig_sub.configVersion = configVersion;
    //payload_manager.payload_deviceConfig_sub.configVersion = doc["configVersion"];
    Serial.print("payload_deviceConfig_sub.configVersion:"); Serial.println(payload_manager.payload_deviceConfig_sub.configVersion);
  }//end      if(doc["configVersion"]
  ...
  if(doc["scheduleVersion"].is<uint8_t>()) {
    scheduleVersion = doc["scheduleVersion"];
    Serial.print("scheduleVersion:"); Serial.println(scheduleVersion);
  ...
  if(doc["defaultDetectionMode"].is<uint8_t>()){
     defaultDetectionMode =  doc["defaultDetectionMode"];
     Serial.print("defaultDetectionMode");Serial.println(defaultDetectionMode);
  ...
  if(doc["lampStartupState"].is<String>()){
    String  lampStartupState =doc["lampStartupState"];
    this->lampStartupState = lampStartupState;
    Serial.print("lampStartupState:");Serial.println(lampStartupState);
  ...
  if (doc["telemetryIntervalSeconds"].is<int>() ){
      telemetryIntervalSeconds=doc["telemetryIntervalSeconds"];
      Serial.print("telemetryIntervalSeconds:");Serial.println(telemetryIntervalSeconds);
  ...
  if( doc["firmwareAutoUpdate"].is<bool>() ){
    firmwareAutoUpdate  = doc["firmwareAutoUpdate"];
    Serial.print("firmwareAutoUpdate:"); Serial.println(firmwareAutoUpdate);
  ...
  if( doc["diagnosticsLevel"].is<uint8_t>()   ){
    diagnosticsLevel =  doc["diagnosticsLevel"];
    Serial.print("diagnosticsLevel");Serial.println(diagnosticsLevel);
...
//  String telemetryTopic = String(thingName) + "/logging/sub";
